...
phase: 10
wave: 3
title: "Write tests/utils/test_csproj_resolver.py — unit + integration tests"
depends_on: [10-01]
files_modified:
  - tests/utils/test_csproj_resolver.py
...
Write comprehensive unit tests for `app/utils/csproj_resolver.py` covering all decision branches (D-01, D-02, D-03 from CONTEXT.md). Tests use `tmp_path` (pytest fixture) — no real dotnet install needed for unit coverage. One integration marker tests that `upgrade_solution` produces valid XML that `ET.parse` can re-read.

---

## Tasks

### Task 1: Create `tests/utils/__init__.py` (if missing)

<read_first>
- tests/ directory listing — run `ls tests/` to see current structure before creating
</read_first>

<action>
Check if `tests/utils/` directory and `tests/utils/__init__.py` exist:
```bash
ls tests/utils/__init__.py 2>/dev/null || echo "MISSING"
```
If missing, create `tests/utils/__init__.py` as empty file.
</action>

<acceptance_criteria>
- `test -f tests/utils/__init__.py && echo OK` outputs `OK`
</acceptance_criteria>

---

### Task 2: Create `tests/utils/test_csproj_resolver.py`

<read_first>
- app/utils/csproj_resolver.py (full file — understand every public function's signature and exceptions)
- tests/ (look at an existing test file for fixture style patterns this project uses)
</read_first>

<action>
Create `tests/utils/test_csproj_resolver.py` with the following content:
...
class TestResolveGraph:
    ...
    def test_simple_chain_a_b_c(self, tmp_path):
        """A → B → C: result must be [C, B, A] (dependencies first)."""
...
<acceptance_criteria>
- `test -f tests/utils/test_csproj_resolver.py && echo OK` outputs `OK`
- `pytest tests/utils/test_csproj_resolver.py -v --tb=short` exits 0 with all tests passing
- `pytest tests/utils/test_csproj_resolver.py --co -q | grep "test session starts" -A 50` shows at least 18 test items collected
- `pytest tests/utils/test_csproj_resolver.py --cov=app/utils/csproj_resolver --cov-report=term-missing` shows ≥ 85% statement coverage
</acceptance_criteria>
...
```bash
pytest tests/utils/test_csproj_resolver.py -v --tb=short
pytest tests/utils/test_csproj_resolver.py --cov=app/utils/csproj_resolver --cov-report=term-missing
```

## must_haves

- [ ] All test classes present: `TestParseSln`, `TestResolveGraph`, `TestResolveFromEntry`, `TestUpgradeTargetFramework`, `TestUpgradeSolution`
- [ ] Circular reference test present and terminates
- [ ] Diamond graph test verifies no duplicate paths
- [ ] Legacy format test verifies `ValueError` with `"ToolsVersion"` in message
- [ ] Post-upgrade XML validity test (re-parse with `ET.parse` must not raise)
- [ ] All tests use `tmp_path` — no real filesystem side effects
- [ ] Test suite passes with `pytest tests/utils/test_csproj_resolver.py` exit code 0
